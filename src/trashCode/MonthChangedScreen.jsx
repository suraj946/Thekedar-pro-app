...
import { getAppOpenDate, setAppOpenDate } from '../utils/asyncStorage';

const {monthIndex:currMonthIndex, dayDate} = getCurrentNepaliDate();

const MonthChangedScreen = ({navigation}) => {
  ...
  useEffect(() => {
    (async () => {
      const prevAppOpenDate = await getAppOpenDate();
      if(!prevAppOpenDate){
        await setAppOpenDate({date:dayDate});
        return;
      }else{
