...
const WorkerCard = ({
  ...
  return (
    <TouchableOpacity onPress={handlePress} activeOpacity={0.9} style={{...styles.container, backgroundColor:selected?light:white}}>
        <View style={styles.textView}>
            <Avatar.Icon icon={"account"} size={moderateScale(40)} style={{marginRight:10}} />
            <View>
                <Text style={styles.nameTxt}>{name}</Text>
                <Text style={styles.roleTxt}>{role}</Text>
            </View>
        </View>
        {selected && <Icon source={"check-circle"} size={moderateScale(25)} color={success} />}
    </TouchableOpacity>
  )
}
